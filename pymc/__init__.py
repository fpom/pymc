...
class CTL_model_checker:
    ...
    def __init__(self, universe, pred, atom=None):
        ...
        self.EG = lambda phi: self.gfp(
            lambda Z: phi & (self.EX(Z) | self.deadlock))
        self.EU = lambda phi1, phi2: self.lfp(
            lambda Z: phi2 | (phi1 & self.EX(Z)))
        self.EW = lambda phi1, phi2: self.gfp(
            lambda Z: phi2 | (phi1 & (self.EX(Z) | self.deadlock)))
        self.ER = lambda phi1, phi2: self.gfp(
            lambda Z: phi2 & (phi1 | self.EX(Z) | self.deadlock))
        self.EM = lambda phi1, phi2: self.lfp(
            lambda Z: phi2 & (phi1 | self.EX(Z)))
        self.AX = lambda phi: (self.EX(self.CTL_True)
                               & self.neg(self.EX(self.neg(phi))))
        self.AF = lambda phi: self.lfp(lambda Z: phi | self.AX(Z))
        self.AG = lambda phi: self.gfp(
            lambda Z: phi & (self.AX(Z) | self.deadlock))
        self.AU = lambda phi1, phi2: self.lfp(
            lambda Z: phi2 | (phi1 & self.AX(Z)))
        self.AW = lambda phi1, phi2: self.gfp(
            lambda Z: phi2 | (phi1 & (self.AX(Z) | self.deadlock)))
        self.AR = lambda phi1, phi2: self.gfp(
            lambda Z: phi2 & (phi1 | self.AX(Z) | self.deadlock))
        self.AM = lambda phi1, phi2: self.lfp(
            lambda Z: phi2 & (phi1 | self.AX(Z)))
        self.unarymod = {"EX": self.EX,
                         "EF": self.EF,
                         "EG": self.EG,
                         "AX": self.AX,
                         "AF": self.AF,
                         "AG": self.AG}
        self.binarymod = {"EU": self.EU,
                          "ER": self.ER,
                          "EW": self.EW,
                          "EM": self.EM,
                          "AU": self.AU,
    ...
    def _phi2sdd(self, phi):
        if phi.kind == 'bool':
            if not isinstance(phi.value, bool):
                raise TypeError(f"{phi!r} is of kind bool"
                                f" but its value is not a boolean")
            if phi.value:
            ...
            return reduce(sdd.__and__,
                          [self._phi2sdd(child)
                           for child in phi.children],
                          self.CTL_True)
        elif phi.kind == 'or':
            return reduce(sdd.__or__,
                          [self._phi2sdd(child) for child in phi.children],
                          self.CTL_False)
        elif phi.kind == 'imply':
            return (self.neg(self._phi2sdd(phi.children[0]))
                    | self._phi2sdd(phi.children[1]))
        elif phi.kind == 'iff':
            return ((self._phi2sdd(phi.children[0])
                     & self._phi2sdd(phi.children[1]))
            ...
            return self.binarymod[phi.kind](self._phi2sdd(phi.children[0]),
                                            self._phi2sdd(phi.children[1]))
        else:
    ...
    def check(self, formula):
        ...
        if not isinstance(formula, (str, Phi)):
            raise TypeError("The formula given in input must be a string"
                            " or a parsed formula")
...
class FARCTL_model_checker(CTL_model_checker):
    def __init__(self, universe, actions, tau_label="_None", atom=None):
        ...
        for action, labels in self.actions.items():
            if not isinstance(action, shom):
                raise TypeError("the key of every item of action"
                                " must be of type shom")
            if not isinstance(labels, list):
                raise ValueError("the value of every item of action"
                                 " must be of type list")
            for lbl in labels:
                if not (isinstance(lbl, str)):
                    raise TypeError("the value of every item of action"
        ...
        super().__init__(universe,
                         reduce(shom.__or__, self.actions),
                         atom)
        self.logic = "FARCTL"
    ...
    def alpha_parse(self, alpha, labels):
        if self.tau_label in labels:
            ...
            if not isinstance(alpha.value, bool):
                raise TypeError(f"{alpha!r} is of kind bool but its value"
                                " is not a boolean")
            return alpha.value
    ...
    def build_pred_alpha(self, alpha):
        return reduce(shom.__or__,
                      [action for (action, labels) in self.actions.items()
    ...
    def _EXevent(self, alpha, event):
        if event.kind == "actions":
            ...
            mc = CTL_model_checker(self.CTL_True,
                                   self.build_pred_alpha(alpha),
                                   self._atom_arg)
            return lambda phi: (self._phi2sdd(event)
                                & (mc.unarymod["EX"](phi)
                                   | mc.deadlock))
    ...
    def _EXnotevent(self, alpha, event):
        if event.kind == "actions":
            ...
            #    \lor \neg\exists_\alpha \X \top
            pr1 = self.build_pred_alpha(Phi("and", alpha,
                                            Phi("not", event.children[0])))
            ...
            return lambda phi: (self.neg(self._phi2sdd(event))
                                & (mc.unarymod["EX"](phi)
                                   | mc.deadlock))
    ...
    def _fairbinarymod(self, pred, EG_fair):
        ...
        EM_fair = mc.EM
        AU_fair = lambda phi1, phi2: mc.neg(EU_fair(mc.neg(phi2),
                                                    mc.neg(phi1)
                                                    & mc.neg(phi2))) \
            & mc.neg(EG_fair(mc.neg(phi2)))
        AW_fair = lambda phi1, phi2: mc.neg(EU_fair(mc.neg(phi2),
                                                    mc.neg(phi1)
                                                    & mc.neg(phi2)))
        ...
        if phi.ufair or phi.wfair or phi.sfair:
            for f in phi.sfair:
                if f.condition.kind != "actions":
                    raise ValueError("Action event not allowed in condition"
                                     " of strong fairness")

            def tau_ufair(Z):
                items = [CTL_model_checker(self.CTL_True, pred)
                         .binarymod["EU"](Z, Z & self._EXevent(alpha,
                                                               f.then)(Z))
                         for f in phi.ufair]
            ...
            def tau_wfair(Z):
                items = [CTL_model_checker(self.CTL_True, pred)
                         .binarymod["EU"](
                             Z, Z & (self._EXnotevent(alpha, f.condition)(Z)
                                     | self._EXevent(alpha, f.then)(Z)))
                         for f in phi.wfair]
                return reduce(sdd.__and__, items, self.CTL_True)

            def tau_sfair(Z):
                items = [self._EXnotevent(alpha, f.condition)(Z)
                         | CTL_model_checker(self.CTL_True, pred)
                         .binarymod["EU"](Z, Z & self._EXevent(alpha,
                                                               f.then)(Z))
                         for f in phi.sfair]
                return reduce(sdd.__and__, items, self.CTL_True)
            ...
            EG_fair = lambda phi: (
                CTL_model_checker(self.CTL_True, pred)
                .binarymod["EU"](phi, self.gfp(
                    lambda Z: phi & tau_ufair(Z) & tau_wfair(Z)
                    & tau_sfair(Z))))
